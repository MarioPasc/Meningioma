...
def icc_bars(ax: plt.Axes, df: pd.DataFrame, palette: Dict[str,str], title: str, roi: str = "all") -> None:
    ...
    # compute medians per cell across features
    agg = df.groupby(["model","resolution"], as_index=False)["ICC2_1"].median()
    x = np.arange(len(res_order))
    width = 0.8 / max(len(models), 1)
    for j, m in enumerate(models):
        sub = agg[agg["model"] == m]
        sub.index = sub["resolution"].astype(int)
        vals = [sub.loc[r, "ICC2_1"] if r in sub.index else np.nan for r in res_order]
        offs = x - 0.4 + j*width + width/2
        ax.bar(offs, vals, width=width, color=palette.get(m, "#555555"), label=m, edgecolor="none", alpha=0.9)
    ax.axhline(0.9, color="#666666", lw=0.8, ls="--"); ax.text(0.01, 0.91, "ICC=0.90", transform=ax.get_yaxis_transform())
    ax.set_ylim(0, 1.0)
    ax.set_xticks(x)
    ax.set_xticklabels([f"{r} mm" for r in res_order])
...
def make_panels_for_pulse(pulse: str, stats: Dict[str, object], out_dir: pathlib.Path) -> None:
    ...
    # --- Panel A (always created): forest + adjusted means with a single bottom legend ---
    figA, axesA = plt.subplots(1, 2, figsize=(7.0, 3.2), constrained_layout=True)
    items = forest_contrasts(axesA[0], contr, palette, title=f"{pulse.upper()}: Effect vs BSPLINE")
    handles, labels = adjusted_means_panel(axesA[1], emm, palette, title=f"{pulse.upper()}: Adjusted means")
    # single figure-level legend at bottom center
    if handles and labels:
        figA.legend(handles, labels, loc="lower center", ncol=len(labels), frameon=False,
                    bbox_to_anchor=(0.5, -0.02))
        figA.subplots_adjust(bottom=0.20)
    for ext in ("pdf","png"):
        out = out_dir / f"sr_panel_{pulse}_A.{ext}"
    ...
    if icc is None or icc.empty:
        ...
        for ext in ("pdf","png"):
            out = out_dir / f"sr_panel_{pulse}_B.{ext}"
            figB.savefig(out); LOG.info("Saved %s", out)
        plt.close(figB)

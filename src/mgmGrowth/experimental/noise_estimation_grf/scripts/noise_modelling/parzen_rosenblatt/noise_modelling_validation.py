...
from mgmGrowth.image_processing import ImageProcessing
from mgmGrowth.metrics import Metrics
import pandas as pd
import numpy as np
...
SIGMA_VALUES = [
    ...
    11,
    11.5,
]  # Sigma values for Rician distribution
...
def plot_distributions(patient_ids: List[int], target_pulse: str):
    for pulse in PULSE_TYPES:
        for patient_id in patient_ids:
            ...
            slices = [
                ImageProcessing.extract_transversal_slice(
                    image_data,
                    transversal_axis,
                    slice_index=mid_slice_idx + offset,
                )
            ...
            x_values = np.linspace(
                np.min(test_noise_data), np.max(test_noise_data), 1000
            )
            ...
            truncated_reds = truncate_colormap(
                plt.cm.Reds,
                min_val=np.min(H_VALUES) * 0.5,
                max_val=np.max(H_VALUES),
            )
            truncated_blues = truncate_colormap(
                plt.cm.Blues,
                min_val=np.min(H_VALUES) * 0.5,
                max_val=np.max(SIGMA_VALUES),
            )
            ...
            norm_sigma = mcolors.Normalize(
                vmin=min(SIGMA_VALUES), vmax=max(SIGMA_VALUES)
            )
            for sigma, rician_pdf in rician_models.items():
                ax.plot(
                    x_values,
                    rician_pdf,
                    color=truncated_blues(norm_sigma(sigma)),
                )
            ...
            ax.legend(
                custom_lines,
                ["Test", "Parzen-Rosenblatt KDE", "Rician"],
                loc="lower center",
            ...
            ax.set_title(
                f"Noise Distribution Comparison for Pulse {pulse} (Patient {patient_id})"
            )
            ...
            plot_path = os.path.join(
                OUTPUT_FOLDER,
                f"{pulse}_distribution_plot_patient_{patient_id}.png",
            )
            plot_path = os.path.join(
                OUTPUT_FOLDER,
                f"{pulse}_distribution_plot_patient_{patient_id}.pdf",
            )
...
def perform_test_normal(patient_ids: List[int]):
        ...
        for patient_id in patient_ids:
            try:
                ...
                if not os.path.exists(pulse_directory):
                    logging.warning(
                        f"Pulse {pulse} is not available for Patient {patient_id}. Skipping this patient."
                    )
                ...
                try:
                    ...
                    logging.info(
                        f"Successfully loaded image for Patient {patient_id} and Pulse {pulse} in {elapsed_time:.4f} seconds"
                    )
                except Exception as e:
                    logging.error(
                        f"Failed to load image for Patient {patient_id} and Pulse {pulse}: {str(e)}"
                    )
                ...
                try:
                    ...
                    end_idx = min(
                        mid_slice_idx + N_SLICES + 1, total_slices
                    )  # +1 because the end index is exclusive
                    slices = [
                        ImageProcessing.extract_transversal_slice(
                            image_data, transversal_axis, slice_index=index
                        )
                    ...
                    elapsed_time = end_time - start_time
                    logging.info(
                        f"Extracted {len(slices)} slices for Patient {patient_id} in {elapsed_time:.4f} seconds"
                    )

                    # Log the number of slices taken and the total number of slices
                    logging.info(
                        f"Taken {len(slices)} slices from {total_slices} total slices for Patient {patient_id}"
                    )
                    ...
                    logging.error(
                        f"Failed to extract slices for Patient {patient_id}: {str(e)}"
                    )
                    results_df.loc[len(results_df)] = [
                        pulse,
                        patient_id,
                ...
                try:
                    ...
                    elapsed_time = end_time - start_time
                    logging.info(
                        f"Train-test split successful for Patient {patient_id} in {elapsed_time:.4f} seconds"
                    )
                except Exception as e:
                    logging.error(
                        f"Failed to split slices for Patient {patient_id}: {str(e)}"
                    )
                    results_df.loc[len(results_df)] = [
                        pulse,
                        patient_id,
                ...
                try:
                    ...
                    elapsed_time = end_time - start_time
                    logging.info(
                        f"Training data collection successful for Patient {patient_id} in {elapsed_time:.4f} seconds"
                    )
                except Exception as e:
                    logging.error(
                        f"Failed to collect training noise data for Patient {patient_id}: {str(e)}"
                    )
                    results_df.loc[len(results_df)] = [
                        pulse,
                        patient_id,
                ...
                try:
                    ...
                    elapsed_time = end_time - start_time
                    logging.info(
                        f"Testing data collection successful for Patient {patient_id} in {elapsed_time:.4f} seconds"
                    )
                except Exception as e:
                    logging.error(
                        f"Failed to collect testing noise data for Patient {patient_id}: {str(e)}"
                    )
                    results_df.loc[len(results_df)] = [
                        pulse,
                        patient_id,
                ...
                try:
                    ...
                    x_values = np.linspace(
                        np.min(train_noise_data),
                        np.max(train_noise_data),
                        1000,
                    )
                    end_time = time.perf_counter()
                    elapsed_time = end_time - start_time
                    logging.info(
                        f"KDE models trained successfully for Patient {patient_id} in {elapsed_time:.4f} seconds"
                    )
                except Exception as e:
                    logging.error(
                        f"Failed to train KDE models for Patient {patient_id}: {str(e)}"
                    )
                    results_df.loc[len(results_df)] = [
                        pulse,
                        patient_id,
                ...
                try:
                    ...
                    elapsed_time = end_time - start_time
                    logging.info(
                        f"Rician models created successfully for Patient {patient_id} in {elapsed_time:.4f} seconds"
                    )
                except Exception as e:
                    logging.error(
                        f"Failed to create Rician models for Patient {patient_id}: {str(e)}"
                    )
                    results_df.loc[len(results_df)] = [
                        pulse,
                        patient_id,
                ...
                try:
                    ...
                    for h, kde_pdf in kde_models.items():
                        ...
                        logging.info(
                            f"Evaluated KDE model with h={h} for Patient {patient_id} in {elapsed_time:.4f} seconds"
                        )
                        start_time = (
                            time.perf_counter()
                    ...
                    logging.error(
                        f"Failed to evaluate Parzen-Rosenblatt models for Patient {patient_id}: {str(e)}"
                    )
                    results_df.loc[len(results_df)] = [
                        pulse,
                        patient_id,
                ...
                try:
                    ...
                    for sigma, rician_pdf in rician_models.items():
                        ...
                        logging.info(
                            f"Evaluated Rician model with sigma={sigma} for Patient {patient_id} in {elapsed_time:.4f} seconds"
                        )
                        start_time = (
                            time.perf_counter()
                        )  # Reset start time for next iteration
                    ...
                    logging.error(
                        f"Failed to evaluate Rician models for Patient {patient_id}: {str(e)}"
                    )
                    results_df.loc[len(results_df)] = [
                        pulse,
                        patient_id,
                ...
                logging.info(
                    f"Total time for Patient {patient_id}: {patient_elapsed_time:.2f} seconds"
                )

            except Exception as e:
                logging.critical(
                    f"Unexpected error for Patient {patient_id} and Pulse {pulse}: {str(e)}"
                )
                results_df.loc[len(results_df)] = [
                    pulse,
                    patient_id,
    ...
    logging.info(
        f"Total execution time for perform_test: {total_elapsed_time:.2f} seconds"
    )
        ...
        for patient_id in patient_ids:
            try:
                ...
                try:
                    ...
                    slices = [
                        ImageProcessing.extract_transversal_slice(
                            image_data, transversal_axis, index
                        )
                        for index in range(start_idx, end_idx)
                    ]
                    elapsed_time = time.perf_counter() - start_time
                ...
                try:
                    np.savez(
                        f"{save_folder}/train_noise_data.npz",
                        data=train_noise_data,
                    )
                    np.savez(
                        f"{save_folder}/test_noise_data.npz",
                        data=test_noise_data,
                    )
                    np.savez(f"{save_folder}/x_values.npz", data=x_values)
                    np.savez(
                        f"{save_folder}/kde_models.npz",
                        **{str(h): kde for h, kde in kde_models.items()},
                    )
                    np.savez(
                    ...
                    logging.error(
                        f"Failed to save data for Patient {patient_id} and Pulse {pulse}: {str(e)}"
                    )
                    continue

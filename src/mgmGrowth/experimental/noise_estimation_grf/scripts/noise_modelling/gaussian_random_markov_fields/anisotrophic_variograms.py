...
def directional_variogram_map(
    ...
    for i, angle in tqdm(
        iterable=enumerate(angles),
        desc="Computing anisotrophic variogram for each angle ...",
        total=len(angles),
        colour="green",
    ):
...
def plot_variogram_polar(
    ...
    A, R = np.meshgrid(
        distances, angles
    )  # 'xy' indexing: A.shape, R.shape => (#angles, #distances)
    # Here:
    #   R[i, j] = angles[i]
    #   A[i, j] = distances[j]
...
def main():
    ...
    slice_data = nrrd2npz.load_mri_slice(filepath=filepath, slice_index=slice_index)

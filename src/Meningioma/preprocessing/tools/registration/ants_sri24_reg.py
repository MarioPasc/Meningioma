...
def register_to_sri24(
    ...
    output_transform_prefix: Optional[str] = None,
    ...
    Args:
        ...
        output_transform_prefix (str, optional):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if verbose:
            ...
            print(f"[ANTS REGISTRATION] Creating temporary directory: {tmpdir}")

        # 1) Save input images as NIfTI
        moving_nii_path = os.path.join(tmpdir, "moving.nii.gz")
        fixed_nii_path = os.path.join(tmpdir, "fixed.nii.gz")
        ...
        # Output paths
        if output_transform_prefix:
            transform_prefix = output_transform_prefix
        else:
            transform_prefix = os.path.join(tmpdir, "transform_")

        if verbose:
            print(f"[ANTS REGISTRATION] Transform prefix set to: {transform_prefix}")

        reg.inputs.output_transform_prefix = transform_prefix
        reg.inputs.output_warped_image = os.path.join(tmpdir, "registered.nii.gz")
        ...
        if verbose:
            ...
            print(
                f"[ANTS REGISTRATION] Running with transforms: {reg.inputs.transforms}"
            )
            print(
                f"[ANTS REGISTRATION] Using histogram matching: {use_histogram_matching}"
            )
        ...
        if verbose:
            print(
                f"[ANTS REGISTRATION] Loading registered image from: {registered_path}"
            )
...
def register_to_sri24_with_mask(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        if verbose:
            ...
            print(
                f"[ANTS REGISTRATION] Creating temporary directory for mask processing: {tmpdir}"
            )
            start_time = time.time()

        # Save mask as NIfTI
        mask_path = os.path.join(tmpdir, "mask.nii.gz")
        output_mask_path = os.path.join(tmpdir, "registered_mask.nii.gz")

        if verbose:
            print(f"[ANTS REGISTRATION] Saving mask to: {mask_path}")
...
def apply_transform_to_image(
    ...
    reference_image_sitk: sitk.Image,
    transform_file: str,
    interpolation: str = "Linear",
    verbose: bool = False,
) -> sitk.Image:
    """
    Apply a previously computed transformation to an input image.

    Args:
        input_image_sitk (sitk.Image):
            The input image to transform as a SimpleITK image.
        reference_image_sitk (sitk.Image):
            The reference image defining the space to transform into.
        transform_file (str):
            Path to the transform file to apply.
        interpolation (str, optional):
            Interpolation method to use. Options include 'Linear', 'NearestNeighbor',
            'BSpline', etc. Default is 'Linear'.
        verbose (bool, optional):
            If True, prints debug info.

    Returns:
        sitk.Image: The transformed input image.
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        if verbose:
            print(
                "[ANTS REGISTRATION] Starting application of transformation to volume"
            )
            print(f"[ANTS REGISTRATION] Creating temporary directory: {tmpdir}")
            start_time = time.time()

        # Save input images as NIfTI
        input_path = os.path.join(tmpdir, "input.nii.gz")
        reference_path = os.path.join(tmpdir, "reference.nii.gz")
        output_path = os.path.join(tmpdir, "transformed.nii.gz")

        if verbose:
            print(f"[ANTS REGISTRATION] Saving input image to: {input_path}")
            print(f"[ANTS REGISTRATION] Saving reference image to: {reference_path}")

        sitk.WriteImage(input_image_sitk, input_path)
        sitk.WriteImage(reference_image_sitk, reference_path)

        # Configure ApplyTransforms
        if verbose:
            print("[ANTS REGISTRATION] Configuring ApplyTransforms")
            print(f"[ANTS REGISTRATION] Using transform: {transform_file}")
            print(f"[ANTS REGISTRATION] Using interpolation: {interpolation}")

        at = ApplyTransforms()
        at.inputs.dimension = 3
        at.inputs.input_image = input_path
        at.inputs.reference_image = reference_path
        ...
        if verbose:
            print(
                "[ANTS REGISTRATION] Applying transformation - this may take a moment..."
            )
            stage_start = time.time()
            at.terminal_output = "stream"
        ...
        if verbose:
            stage_end = time.time()
            print(
                f"[ANTS REGISTRATION] Transformation applied in {stage_end - stage_start:.2f} seconds"
            )
            print(
                f"[ANTS REGISTRATION] Transformed image saved to: {at_result.outputs.output_image}"
            )

        # Load the transformed image
        transformed_sitk = sitk.ReadImage(at_result.outputs.output_image)
        transformed_sitk.CopyInformation(reference_image_sitk)

        if verbose:
            end_time = time.time()
            print(
                f"[ANTS REGISTRATION] Total transformation process completed in {end_time - start_time:.2f} seconds"
            )

        return transformed_sitk
...
def main():
    ...
    if args.mask_path:
        ...
        registered_image, registered_mask, transform_params = (
            register_to_sri24_with_mask(
                ...
                output_transform_prefix=args.output_transform,
        ...
        if args.verbose:
            print(
                f"[ANTS REGISTRATION] Saving registered image to: {args.output_registered}"
            )

        sitk.WriteImage(registered_image, args.output_registered)

        if args.output_mask:
            if args.verbose:
                print(
                    f"[ANTS REGISTRATION] Saving registered mask to: {args.output_mask}"
                )

            sitk.WriteImage(registered_mask, args.output_mask)
    ...
    if args.verbose:
        ...
        print(
            f"[ANTS REGISTRATION] Transform files saved with prefix: {args.output_transform}"
        )
        for k, v in transform_params.items():
            if v:
                print(f"[ANTS REGISTRATION]   {k}: {v}")

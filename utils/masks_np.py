...
class NRRDToNPZConverter:
    ...
    def reorganize_axes(self, images: np.ndarray, masks: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        try:
    ...
    def combine_images_and_masks(self, images: np.ndarray, masks: np.ndarray) -> np.ndarray:
        try:
    ...
    def save_to_npz(self, combined: np.ndarray, patient_id: str, sequence_type: str) -> None:
        try:
            ...
            raise IOError(f"Error saving NPZ file for patient {patient_id}, sequence {sequence_type}: {e}")
    ...
    def convert_patient_data(self, patient_id: str) -> None:
        try:
            ...
            sequences = [os.path.join(patient_path, seq) for seq in os.listdir(patient_path) if os.path.isdir(os.path.join(patient_path, seq))]
            ...
            for seq in sequences:
                ...
                try:
                    ...
                    images_reordered, masks_reordered = self.reorganize_axes(images, masks)
                    ...
                    if images_reordered.shape != masks_reordered.shape:
                        raise ValueError(f"Shapes do not match for patient {patient_id}, sequence {sequence_type}")

                    combined = self.combine_images_and_masks(images_reordered, masks_reordered)
    ...
    def convert_all_patients(self) -> None:
        try:
            patients = [d for d in os.listdir(self.base_ds_path) if os.path.isdir(os.path.join(self.base_ds_path, d))]

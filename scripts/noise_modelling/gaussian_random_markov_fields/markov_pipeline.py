...
def experiment():
    ...
    im_kspace = axes[0, 1].imshow(
        np.log1p(np.abs(k_space)), cmap="gray", origin="lower"
    )
    ...
    plt.savefig(
        "scripts/noise_modelling/gaussian_random_markov_fields/MRF_slices.svg",
        format="svg",
    )
    ...
    for (model_name, (model, stats)), color in zip(results.items(), colors):
        ...
        handles.append(
            plt.Line2D(
                ...
                color=color,
                linestyle="--",
                label=f"{model_name} (R²={stats['r2']:.4f})",
            )
        )
    ...
    handles.append(
        plt.Line2D(
            [0], [0], color="red", linestyle="-", label=f"Best Model: {best_model_name}"
        )
    )
    ...
    plt.savefig(
        "scripts/noise_modelling/gaussian_random_markov_fields/MRF_variogram_fitting.svg",
        format="svg",
    )

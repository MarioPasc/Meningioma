...
class DatasetTransformerAdquisition:
        ...
        for type in self.adquisition_types:
            ...
            if type == "RM":
                for pulse in self.rm_pulses:
                    os.makedirs(
                        os.path.join(self.root, type, pulse), exist_ok=True
                    )
    ...
    def _move_RM_Pulse(self, pulse: str, verbose: bool = False) -> None:
        ...
        for patient_folder in tqdm(
            iterable=self.patient_folders,
            desc=f"Moving {pulse} pulse ...",
            ...
            if os.path.exists(
                pulse_path
            ):  # Only process if the pulse exists for that patient
                files = os.listdir(pulse_path)
                seg = False

                if (
                    len(files) == 0
                ):  # Some patients have the pulse folder but nothing inside
                    if verbose:
                        print(
                            f"Patient {patient_folder} does not have {pulse} pulse (missing files). \n============"
                        )
                else:
                    if "Segmentation.nrrd" in files:  # Control patients
                        segmentation = files.pop(
                            files.index("Segmentation.nrrd")
                        )
                        seg = True
                    else:
                        if verbose:
                            print(
                                f"Patient {patient_folder} with {pulse} pulse but without Segmentation.nrrd"
                            )

                    image = [
                        file
                        for file in files
                        if not file.endswith((".png", ".seg.nrrd", ".mrml"))
                    ][0]
                    if verbose:
                        print(
                            f"Patient: {patient_folder}\n Segmentation: {segmentation}\n File: {image}\n============"
                        )
                    ...
                    stripped = re.sub(
                        r"tc", "", patient_folder, flags=re.IGNORECASE
                    )
                    target_pulse_folder = os.path.join(
                        self.root, "RM", pulse, f"P{stripped}"
                    )  # Strip the TC at the end
                    ...
                    image_destination = os.path.join(
                        target_pulse_folder, new_image_name
                    )
                    ...
                    if seg:  # If the image has a segmentation file ...
                        ...
                        segmentation_source = os.path.join(
                            pulse_path, segmentation
                        )
                        segmentation_destination = os.path.join(
                            target_pulse_folder, new_segmentation_name
                        )
                        shutil.copy2(
                            segmentation_source, segmentation_destination
                        )

            else:
                if verbose:
                    print(
                        f"Patient {patient_folder} does not have {pulse} pulse (missing folder).\n============"
                    )
    ...
    def _move_TC(self, verbose: bool = False) -> None:
        ...
        for patient in tqdm(
            ...
            if not os.path.exists(
                tc_root
            ):  # Some patients may have TC missing
                if verbose:
                    print(
                        f"Patient {patient} does not have TC study (missing folder)."
                    )
                ...
                if len(files) == 0:
                    if verbose:
                        print(
                            f"Patient {patient} does not have TC study (missing files)."
                        )
                    ...
                    if "Segmentation.nrrd" not in files:
                        if verbose:
                            print(
                                f"Patient {patient} without Segmentation.nrrd"
                            )
                    ...
                    image = [
                        ...
                        if (not file.endswith((".png", ".seg.nrrd", ".mrml")))
                        and (
                            os.path.isfile(os.path.join(tc_root, file))
                            and file.endswith(".nrrd")
                        )
                    ]
                    if image:
                        image = image[0]
                    ...
                    target_TC_folder = os.path.join(
                        self.root, "TC", f"P{patient}"
                    )
                    ...
                    image_destination = os.path.join(
                        target_TC_folder, new_image_name
                    )
                    shutil.copy2(image_source, image_destination)
                    ...
                    if seg:  # If the image has a segmentation file ...
                        ...
                        segmentation_source = os.path.join(
                            tc_root, segmentation
                        )
                        segmentation_destination = os.path.join(
                            target_TC_folder, new_segmentation_name
                        )
                        shutil.copy2(
                            segmentation_source, segmentation_destination
                        )
...
def main() -> int:
    ...
    ds_transformer = DatasetTransformerAdquisition(
